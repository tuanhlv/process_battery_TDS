...
def test_missing_cycles(mock_to_csv, mock_savefig):
    rawcsv_fn = "test_missing_cyc.csv"
    rawcsv_path = os.path.join("tests", rawcsv_fn)
    assert os.path.exists(rawcsv_path), f"Test file missing: {rawcsv_path}"  # Fail immediately with a clear error if the dummy CSV is missing 
    cycles = "1, 7"
    rates = "0.2, 10"
